object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factorize(number: Int, divisor: Int): List[Int] = {
            (divisor*divisor < number, number % divisor) match {
                case (true, 0) => divisor :: factorize(number/divisor, divisor)
                case (true, _) => factorize(number, divisor+1)
                case (false, _) => List(number)
            }
        }
        factorize(number, 2).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val sortedMap = ballsArray.toSeq.sortBy(x => x._2._2 * x._2._1 * x._2._1 * Math.PI)
        val sortedNames: Seq[String] = Seq()
        for(ball <- sortedMap) sortedNames :+  ball._1
        sortedNames
    }
